...
struct dynarray* dynarray_create()
{
    ...
    //allocates memory for a new dynarray and assigns it to the dynarray pointer dyna_array
    struct dynarray *dyna_array = malloc(sizeof(struct dynarray));

    //allocates memory for dyna_array's data to have 2 spots in the array for void* types
    dyna_array->data = malloc(2 * sizeof(void*));

    //initializes dyna_array's other member variables size and capacity
    dyna_array->size = 0;
    dyna_array->capacity = 2;
...
void dynarray_free(struct dynarray* da)
{
    ...
    //frees memory held in da's data array as well as the dynarray that da points to
    free(da->data);
    da->data = NULL;
...
void dynarray_insert(struct dynarray* da, void* val)
{
    ...
    if(da->size < da->capacity)
        ...
        //allocates memory for a new data array to have twice the capacity as da's data array
        da->capacity *= 2;
        void** new_data = malloc(da->capacity * sizeof(void*));

        //copies the elements in da's data array to the new data array
        ...
        //inserts val to the index in da's data array that would have been one past the end of da's data array
        da->data[da->size] = val;
    }

    //increments the size of the dynamic array
    da->size++;
...
void dynarray_remove(struct dynarray* da, int idx)
{
    ...
    //shifts all of the elements after idx in the array down one 
    for(int i = idx; i < da->size - 1; i++){

        da->data[i] = da->data[i + 1]; 
    }
    
    //decrements size of dynamic array
    da->size--; 
